...
class DataLoader(object):
	...
	def load_sentences_labels(self, tweets_file, labels_file, d):
		"""
		...
		with open(tweets_file) as f:
			for tweet in f.read().splitlines():
				...
				s = [self.vocab[word] if word in self.vocab 
					 else self.unk_ind
		...
		with open(labels_file) as f:
			...
			for label in f.read().splitlines():
				# we associate the same label to each word in the tweet
				if int(label) == 1:
					labels_1 += 1
				l = [int(label) for j in range(len(tweets[i]))]
				labels.append(l)  
				i += 1
		
		# checks to ensure there is a label for each word
		assert len(labels) == len(tweets)
		for i in range(len(labels)):
			assert len(labels[i]) == len(tweets[i])
	...
	def load_data(self, types, data_dir):
		...
		for split in ['train', 'val', 'test']:
			if split in types:
				tweets_file = os.path.join(data_dir, split, "tweets.txt")
				labels_file = os.path.join(data_dir, split, "labels.txt")
				data[split] = {}
				self.load_sentences_labels(tweets_file, labels_file, data[split])
	...
	def data_iterator(self, data, params, shuffle=False):
		...
		for i in range((data['size']+1)//params.batch_size):
			...
			batch_tweets = [data['data'][idx] for idx in order[i*params.batch_size:(i+1)*params.batch_size]]
			batch_tags = [data['labels'][idx] for idx in order[i*params.batch_size:(i+1)*params.batch_size]]

			# compute length of longest sentence in batch
			batch_max_len = max([len(s) for s in batch_tweets])

			# prepare a numpy array with the data, initialising the data with pad_ind and all labels with -1
			# initialising labels to -1 differentiates tokens with tags from PADding tokens
			batch_data = self.pad_ind*np.ones((len(batch_tweets), batch_max_len))
			batch_labels = -1*np.ones((len(batch_tweets), batch_max_len))

			# copy the data to the numpy array
			for j in range(len(batch_tweets)):
				cur_len = len(batch_tweets[j])
				batch_data[j][:cur_len] = batch_tweets[j]
