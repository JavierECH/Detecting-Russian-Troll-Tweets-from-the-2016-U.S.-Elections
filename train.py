...
def train_and_evaluate(model, train_data, val_data, optimizer, loss_fn, metrics, params, model_dir, restore_file=None):
	...
	for epoch in range(params.num_epochs):
		...
		mean_epoch_loss, epoch_loss = train(model, optimizer, loss_fn, train_data_iterator, metrics, params, num_steps)
	...
	return mean_loss_per_epoch, loss_per_epoch
...
def graph_loss(mean_loss, loss, num_epochs, title):
	...
	bandxaxis = go.XAxis(
		...
		tickvals=[(i+1) * num_steps for i in range(num_epochs)]
	)

	plotly.offline.plot({
		"data": [go.Scatter(y=flat_list1, name='batch loss', line = dict(width=0.2)), 
				go.Scatter(y=flat_list2, name='mean batch loss per epoch', line=dict(width=5))],
		"layout": go.Layout(
			xaxis=bandxaxis,
			yaxis=dict(
				title=''
...
if __name__ == '__main__':
	...
	mean_loss, loss = train_and_evaluate(model, train_data, val_data, optimizer, loss_fn, metrics, params, args.model_dir,
					   args.restore_file)	

	# graph the batch losses during training
	graph_loss(mean_loss, loss, params.num_epochs, '')
